...
# Our refactored solution relies on using .sort_by to sort all the hash's keys by the values. This means at least the first mode is the first key in the sorted hash and means we only have to iterate through the hash until we hit values smaller than that. The method .each lets us increment through all the keys and values of the hash. These particular methods were not difficult to implement, but getting several of the others we tried to work was very frustrating and we wound up down a number of unproductive (but educational) rabbit-holes.